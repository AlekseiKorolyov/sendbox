...
9           Оператор расширения (...) позволяет копировать все элементы из одного массива в другой

11-17       Стрелочная функция

19          Метод map() используется для итерации по массиву и возврата нового массива

24          Метод join() используется для объединения всех элементов массива в одну строку

26          Необязательное связывание (?.) помогает предотвратить ошибки при доступе к вложенным свойствам, которые могут быть пустыми или неопределёнными

28-34       Метод sort() преобразует элементы массива в строки и сортирует их по месту на основе их значений

36          Метод find() извлекает первый элемент массива, удовлетворяющий условиям, указанным в предоставленной функции обратного вызова

38          Перед воспроизведением песни необходимо убедиться, что она начинается с начала. Этого можно добиться, используя свойство currentTime аудиообъекта

41          свойство classList и метод add(), чтобы добавить класс «playing» к элементу playButton. метод play() переменной audio. play() — это метод API веб-аудио для воспроизведения mp3-файлов

42          добавить функционал к кнопке воспроизведения, чтобы она воспроизводила текущую песню при нажатии на неё. Используйте метод addEventListener() и передайте событие «click»

45          Чтобы песня воспроизводилась каждый раз, когда пользователь нажимает на неё, добавьте атрибут onclick

48          pause() — это метод API веб-аудио для приостановки музыкальных файлов.

51          Метод массива indexOf() возвращает первый индекс, по которому находится заданный элемент в массиве, или -1, если элемент отсутствует

61          используйте querySelectorAll для получения элемента

63          Метод forEach используется для прохода по массиву и выполнения функции над каждым элементом массива

64          метод removeAttribute() для удаления атрибута

65          setAttribute для songToHighlight, чтобы передать «aria-current» и «true» в качестве первого и второго аргументов.

70          textContent задаёт текст узла и позволяет задать или получить текстовое содержимое HTML-элемента тернарный оператор

73          оператор OR (||) чтобы присвоить ей текущую песню из userData или первую песню в массиве

74          Используйте метод setAttribute элемента playButton, чтобы установить атрибут

77          Один из способов рандомизации массива элементов — вычесть 0,5 из Math.random(), что даст случайные значения, которые могут быть как положительными, так и отрицательными

82          Метод filter() сохраняет только те элементы массива, которые удовлетворяют переданной ему функции обратного вызова

88          createElement() — это метод DOM, который можно использовать для динамического создания элемента с помощью JavaScript

89          Метод createTextNode() используется для создания текстового узла Чтобы использовать его, вызовите его и передайте текст как строку

90          JavaScript предоставляет необходимые для этого свойства id и ariaLabel. Например, element.id устанавливает атрибут id, а element.ariaLabel — атрибут aria-label

91          метод appendChild(). AppendChild() позволяет добавить узел или элемент в качестве дочернего элемента другого элемента

94          далите кнопку сброса из плейлиста, вызвав метод remove()

95          прослушиватель событий «ended» срабатывает по завершении воспроизведения медиафайла
...
Когда всё готово, вызовите функции pauseSong(), setPlayerDisplay(), highlightCurrentSong() и setPlayButtonAccessibleText() для корректного обновления плеера. Поздравляем с завершением работы над музыкальным плеером! Теперь, когда мы закончили тестирование и использовали короткие песни, мы заменили их оригинальными треками, специально подобранными Куинси для вас.
