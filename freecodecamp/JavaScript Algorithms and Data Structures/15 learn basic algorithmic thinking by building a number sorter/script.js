...
После цикла while вам нужно вставить текущее значение. Помните, что цикл завершается, когда j выходит за пределы массива или когда значение в позиции j меньше вашего текущего значения. Используйте оператор присваивания, чтобы вставить текущее значение в правильный индекс.
...
Кнопка «Сортировка» может казаться работающей правильно при нажатии, но это происходит только потому, что все значения в массиве — однозначные, и сортировка может работать не так, как ожидается, с более сложными значениями. Измените значение и текст выбранного элемента с 1 на 10 и снова нажмите кнопку «Сортировка». (index.html str 80)
...
Обратите внимание, что число 10 находится в начале массива. Это происходит потому, что по умолчанию метод `.sort()` преобразует числовые значения в строки и сортирует их по алфавиту. А «10» стоит перед «2» в алфавитном порядке. Чтобы исправить это, вы можете передать функцию обратного вызова методу `.sort()`. Функция обратного вызова имеет два параметра — в вашем случае используйте `a` и `b`. Параметры `a` и `b` представляют собой числовые значения в массиве, которые будут отсортированы. Пока оставьте функцию пустой.
...
Если вы снова нажмете кнопку «Сортировка», вы увидите, что число 10 теперь находится в правильном положении в выходных данных. Чтобы завершить этот проект, верните значение и текст 1. (index.html str 80)
